...
<template>
    ...
    <BaseForm v-if="step === 1" @handleSubmit="sendOtp">
        <template #input>
            <BaseInputEmail v-model="form.email" customId="email" customPlaceholderInput="Nhập email"
                customClassInput="form-control" :isRequired="true" :error="error.email" />
        </template>

        <template #button>
            <BaseButton v-if="!loading" customText="Gửi OTP" customClass="btn btn-primary" customType="submit" />
            <BaseSpinner v-else customClass="btn btn-primary" />
        </template>
    </BaseForm>
    ...
    <div v-if="step === 2">
        <BaseForm @handleSubmit="verifyOtp">
            <template #input>
                ...
                <!-- nút xác nhận -->
                <BaseButton v-if="!loading" customText="Xác nhận" customClass="btn btn-primary" customType="submit" />
                <BaseSpinner v-else customClass="btn btn-primary" />
            </template>
        </BaseForm>
    </div>
    ...
    <BaseForm v-if="step === 3" @handleSubmit="resetPassword">
        <template #input>
            <BaseInputPassword v-model="form.newPassword" customId="newPassword"
                customPlaceholderInput="Nhập mật khẩu mới" customClassInput="form-control" :isRequired="true"
                :error="error.newPassword" />
            <BaseInputPassword v-model="form.newPassword_confirmation" customId="newPassword_confirmation"
                customPlaceholderInput="Xác nhận mật khẩu mới" customClassInput="form-control" :isRequired="true"
                :error="error.newPassword_confirmation" />
        </template> <template #button>
            <BaseButton v-if="!loading" customText="Đặt lại mật khẩu" customClass="btn btn-primary" customType="submit" />
            <BaseSpinner v-else customClass="btn btn-primary" />
        </template>
    </BaseForm>
