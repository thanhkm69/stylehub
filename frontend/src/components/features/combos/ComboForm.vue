...
<template>
    <BaseModal @close="emit('close')" :isShow="isShow"
        :title="dataForm.id ? 'Cập nhật Combo' : 'Thêm Combo mới'" customWidth="900px">
            ...
            <template #input>
                <div class="combo-form-grid">
                    ...
                    <div class="form-column">
                        <div class="form-section">
                            <h4 class="section-subtitle">
                                <i class="ph-bold ph-stack"></i> Thông tin cơ bản
                            </h4>
                            ...
                            <BaseInputTextarea labelContent="Mô tả" customId="description"
                                v-model="dataForm.description"
                                customPlaceholderInput="Mô tả điều kiện áp dụng combo..." />
                        </div>

                        <div class="form-section">
                            <h4 class="section-subtitle">
                                <i class="ph-bold ph-calendar"></i> Thời gian diễn ra
                            </h4>
                    ...
                    <div class="form-column secondary-column">
                        <div class="form-section">
                            <h4 class="section-subtitle">
                                <i class="ph-bold ph-image"></i> Ảnh đại diện
                            </h4>
                            ...
                            <div class="image-upload-wrapper">
                                ...
                                <BaseInputFile labelContent="" customId="thumbnail" :error="errors.thumbnail"
                                    customAccept="image/*"
                                    @change="(e) => emit('handleImageChange', e)" />
                                <p class="upload-hint">JPG, PNG, WEBP. Tối đa 2MB.</p>
                            </div>
                        </div>

                        <div class="form-section no-border">
                            <h4 class="section-subtitle">
                                <i class="ph-bold ph-sliders"></i> Cấu hình hiển thị
                            </h4>
            ...
            <template #button>
                <div class="form-actions">
                    ...
                    <BaseButton v-if="!loadingSubmit" customType="submit"
                        :customText="dataForm.id ? 'Lưu thay đổi' : 'Tạo Combo'"
                        customClass="btn btn-primary px-5" />
                    <div v-else class="loading-submit"><BaseSpinner size="sm" /></div>
                </div>
            </template>
        </BaseForm>
    </BaseModal>
...
.secondary-column { padding-left: 32px; border-left: 1px solid #f1f5f9; }
.form-section { padding-bottom: 8px; }
.section-subtitle {
    font-size: 14px; font-weight: 700; color: var(--text-main);
    margin-bottom: 20px; display: flex; align-items: center; gap: 8px;
}
.section-subtitle i { color: var(--primary); font-size: 18px; }
.two-col-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
.form-group { margin-bottom: 20px; }
.form-label { display: block; margin-bottom: 8px; font-weight: 500; font-size: 14px; color: var(--text-main); }
.form-control {
    width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 8px;
    background: var(--background); color: var(--text-main); font-size: 14px;
    box-sizing: border-box; transition: border-color 0.2s;
}
.form-control:focus { border-color: var(--primary); outline: none; box-shadow: 0 0 0 3px rgba(99,102,241,0.1); }
.form-error { display: block; margin-top: 4px; font-size: 12px; color: var(--danger); }
...
.image-preview-large {
    ...
    border: 2px dashed var(--border); position: relative; background: #f8fafc;
}
.image-preview-large img { width: 100%; height: 100%; object-fit: contain; }
.image-placeholder {
    width: 100%; height: 180px; border-radius: 16px; border: 2px dashed var(--border);
...
.preview-overlay {
    ...
    color: white; font-size: 11px; padding: 4px 10px; border-radius: 20px;
}
...
.loading-submit { display: flex; align-items: center; padding: 0 40px; }
@media (max-width: 849px) {
    .combo-form-grid { grid-template-columns: 1fr; }
    .secondary-column { padding-left: 0; border-left: none; border-top: 1px solid #f1f5f9; padding-top: 24px; }
}
</style>
