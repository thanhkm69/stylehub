...
<template>
    <BaseAdmin :total="totalItems" :totalPages="totalPages" :currentPage="params.page"
        ...
        <template #table>
            <ComboTable :params="params" :loadingData="loadingData" :data="combos"
                @update="update" @destroy="destroy" @manageItems="manageItems" />
        </template>
    </BaseAdmin>

    <ComboForm v-model:isShow="isShow" v-model:dataForm="dataForm" v-model:loadingSubmit="loadingSubmit"
        :errors="errors" :comboTypes="comboTypes" :discountTypes="discountTypes" :statusMap="statusMap"
        @submit="submit" @close="closeForm" @handleImageChange="handleImageChange" />

    <ComboItemModal v-if="selectedCombo" v-model:isShow="isShowItem" :combo="selectedCombo"
        @itemsChanged="loadData" @close="isShowItem = false" />
</template>
